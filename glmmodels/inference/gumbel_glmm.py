...
class GumbelGLMM(GLMM):
    ...
    def expCondLogProb(self) -> torch.Tensor:
        ...
        return torch.sum( torch.exp(- (self.Y - self.mn)/self.beta + self.Sn_ii/torch.square(self.beta)))
    ...
    def computeEtaLamda(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.nn_model is not None:
            nn_input = torch.cat((self.Y, torch.sigmoid(self.Y)))
    ...
    def init_model_params(self) -> None:
        ...
        # Scale parameter
        self.beta = torch.ones(N, d).to(torch.float64).requires_grad_()
...
if __name__ == '__main__':
    ...
    mu_yi = z_samples # Identity link function
    ...
    gumbel_dist = Gumbel(mu_yi, beta)

    Y = gumbel_dist.sample()

    Y = Y.to(torch.float64)

    # print(Y)

    model = GumbelGLMM(Y.numpy(), X.numpy(), K=K)
